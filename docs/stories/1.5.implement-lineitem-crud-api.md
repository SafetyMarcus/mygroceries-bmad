...
- [ ] Define `Order` data class in `shared` module
    - [ ] Create `Order.kt` in `shared/src/commonMain/kotlin/com/example/mygroceries/model/`
    - [ ] Implement fields with appropriate data types and serialization
    - [ ] Add `lineItems` property for API responses

- [ ] Implement Order repository/DAO for database operations
    - [ ] Create `OrderRepository.kt` in `server/src/main/kotlin/com/example/mygroceries/db/`
    - [ ] Implement `create`, `readAll`, `readById`, `update`, `delete` functions using Exposed
    - [ ] Set up proper foreign key relationships with `line_items` table

- [ ] Implement Order service layer
    - [ ] Create `OrderService.kt` in `server/src/main/kotlin/com/example/mygroceries/service/`
    - [ ] Inject `OrderRepository` and `LineItemRepository`
    - [ ] Implement business logic for CRUD operations, including validation
    - [ ] Implement order total cost calculation and updates

- [ ] Implement Order API routes
    - [ ] Create `OrderRoutes.kt` in `server/src/main/kotlin/com/example/mygroceries/routes/`
    - [ ] Define all required endpoints with proper request/response handling

### LineItem Implementation
- [ ] Define `LineItem` data class in `shared` module
    - [ ] Create `LineItem.kt` in `shared/src/commonMain/kotlin/com/example/mygroceries/model/`
    - [ ] Implement fields with appropriate data types and serialization
    - [ ] Add `product` property for API responses

- [ ] Implement LineItem repository/DAO for database operations
    - [ ] Create `LineItemRepository.kt` in `server/src/main/kotlin/com/example/mygroceries/db/`
    - [ ] Implement `create`, `readAll`, `readById`, `update`, `delete` functions using Exposed
    - [ ] Set up proper foreign key relationships with `orders` and `products` tables
    - [ ] Implement methods to find line items by order ID

- [ ] Implement LineItem service layer
    - [ ] Create `LineItemService.kt` in `server/src/main/kotlin/com/example/mygroceries/service/`
    - [ ] Inject `LineItemRepository`, `OrderRepository`, and `ProductRepository`
    - [ ] Implement business logic for CRUD operations, including validation
    - [ ] Implement order total cost updates when line items change

- [ ] Implement LineItem API routes
    - [ ] Create `LineItemRoutes.kt` in `server/src/main/kotlin/com/example/mygroceries/routes/`
    - [ ] Define all required endpoints with proper request/response handling
    - [ ] Implement nested routes under `/orders/{orderId}/lineitems`

### Testing
- [ ] Add unit tests for `OrderService`
    - [ ] Test validation logic
    - [ ] Test business logic and total cost calculation

- [ ] Add integration tests for Order API endpoints
...
- [ ] Add unit tests for `LineItemService`
    - [ ] Test validation logic
    - [ ] Test business logic and order total updates

- [ ] Add integration tests for LineItem API endpoints
    - [ ] Test successful CRUD operations
    - [ ] Test validation error cases
    - [ ] Test order total updates when line items change
