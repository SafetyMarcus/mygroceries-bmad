...
## Tasks / Subtasks
- [ ] Create database migration files for `categories`, `products`, `orders`, and `line_items` tables (AC: 1)
    - [ ] Define `categories` table with `id` (UUID) and `name` (VARCHAR) [Source: architecture/7-database-schema.md]
    - [ ] Define `products` table with `id` (UUID), `name` (VARCHAR), and `category_id` (UUID, FK to `categories`) [Source: architecture/7-database-schema.md]
    - [ ] Define `orders` table with `id` (UUID), `order_date` (TIMESTAMPTZ), and `total_cost` (NUMERIC) [Source: architecture/7-database-schema.md]
    - [ ] Define `line_items` table with `id` (UUID), `order_id` (UUID, FK to `orders`), `product_id` (UUID, FK to `products`), `cost` (NUMERIC), and `quantity` (DOUBLE PRECISION) [Source: architecture/7-database-schema.md]
    - [ ] Add foreign key constraints and indexes as specified in the schema [Source: architecture/7-database-schema.md]
- [ ] Configure Ktor server to connect to local PostgreSQL database on startup (AC: 3)
    - [ ] Implement connection pooling [Source: architecture/3-tech-stack.md]
- [ ] Implement mechanism for the server to apply database migrations on startup (AC: 4)
- [ ] Verify table schemas reflect data models and relationships (AC: 2)
- [ ] Add unit tests for database connection and migration application [Source: architecture/9-test-strategy-and-standards.md]
