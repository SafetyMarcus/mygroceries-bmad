...
- [ ] Implement integration tests to verify schema creation and basic data persistence (AC 5).
- [ ] Externalize database credentials (username, password) from `DatabaseFactory.kt` using environment variables or a secure configuration system.
- [ ] Update `DatabaseFactoryTest.kt` to use a test PostgreSQL instance or add a separate integration test suite for PostgreSQL-specific behaviors.
- [ ] Refine exception handling in `createDatabaseIfNotExists()` to specifically catch "database already exists" errors and log other exceptions.
