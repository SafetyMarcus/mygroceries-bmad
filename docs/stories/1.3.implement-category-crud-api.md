...
| 2025-10-25 | 1.3 | Addressed QA findings: Extracted validation logic to `CategoryValidator` and added integration tests for error scenarios. | James (Dev) |
| 2025-10-26 | 1.4 | Heavily refactored and actually got both the application and tests building and running. They were not before. | Marcus (Hooper) |
...
- `gradlew :server:check` output (successful run after fixes)
...
- **Medium: Consider extracting validation logic to separate validator class.**
    - Resolution: Extracted validation logic from `CategoryService` into a new `CategoryValidator` class. `CategoryService` now injects and uses `CategoryValidator`.
- **Medium: Add integration test for error scenarios.**
    - Resolution: Added integration tests to `CategoryRoutesTest.kt` to cover invalid UUID formats and empty category names, ensuring appropriate `400 Bad Request` responses.
...
- `mygroceries/server/src/main/kotlin/com/safetymarcus/mygroceries/service/CategoryService.kt` (modified)
- `mygroceries/server/src/main/kotlin/com.safetymarcus.mygroceries.service/CategoryValidator.kt` (added)
- `mygroceries/server/src/main/kotlin/com/safetymarcus/mygroceries/routes/CategoryRoutes.kt`
- `mygroceries/server/src/test/kotlin/com/safetymarcus/mygroceries/service/CategoryServiceTest.kt` (modified)
- `mygroceries/server/src/test/kotlin/com/safetymarcus/mygroceries/db/TestDatabaseFactory.kt`
- `mygroceries/server/src/test/kotlin/com/safetymarcus/mygroceries/routes/CategoryRoutesTest.kt` (modified)
...
- [x] Consider extracting validation logic to separate validator class
